...
# Charger les données
df_portfolios = pd.read_parquet("portefeuilles_q100_eodhd.parquet")
...
df_result.to_csv("resultats_portefeuillesq100.csv", index=False)
print("✅ Résultats sauvegardés dans resultats_portefeuillesq100.csv")

with open("resultats_portefeuilles_latexq100.txt", "w") as f:
    for row in results:
        shape = row["Shape"] if row["Shape"] != "" else " "
        line = f"{row['Year']} & {row['Sample size']} & {shape} & {row['Scale']} & {row['Location']} & {row['AIC']} & {row['PDF']} \\\\"
        f.write(line + "\n")

print("✅ Table LaTeX générée dans resultats_portefeuilles_latexq100.txt")

import matplotlib.pyplot as plt
import seaborn as sns
plt.figure(figsize=(8, 5))
plt.hist(df_result["Mean return"], bins=10, edgecolor='black', alpha=0.7)
plt.title("Average annual return distribution (2012-2024) q=100")
plt.xlabel("Return")
plt.ylabel("Frequence")
plt.grid(True)
plt.tight_layout()
plt.savefig("histogramme_rendements_moyensq100.png")
plt.close()
